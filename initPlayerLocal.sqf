...
if(PT_LOCAL_SCRIPTS == true) then {
	...
	//TODO: THIS SHOULD REALLY BE SERVER-SIDE FOR MULTIPLAYER
	eventHandlerVehicle = {
		params ["_vehicle", "_weapon", "_muzzle", "_mode", "_ammo", "_magazine", "_projectile"];
		...
		({
			...
			if(_vehicle == _mappedVehicle) then {
				...
				// //Grab the designated poop group or designate it for future use
				// _poopGroup = missionNameSpace getVariable "poopGroup";
				...
				//Create unit to launch in capsule
				// _unit = _poopGroup createUnit ["UK3CB_CW_SOV_O_LATE_SF_RIF_2", [0,0,0], [], 0, "FORM"];

				//Delete original projectile we're piggybacking on and set out unit's position to the pod's
				...
				// [_pod, _vehicle, _poopGroup] spawn {COMMENTED TO USE SEPARATE SQF's 03/16 2118 MST
				...
				// 	playSound3D ["rhsusf\addons\rhsusf_a2port_air\data\sounds\ejection_sound.wss", _pod, false, getPosATL _pod, 5, 1];
				...
				// };COMMENTED TO USE SEPARATE SQF's 03/16 2118 MST
